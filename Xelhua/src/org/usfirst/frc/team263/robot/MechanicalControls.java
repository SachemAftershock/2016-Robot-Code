...
public class MechanicalControls {
	...
	public void drive() {
		...
		} else if (gamepad.getRawButton(B_BUTTON) && ballGrabberLS.get() /*&& pot.get() > 150*/ ) {
			// Makes the pivot go down at 50% speed
			// Unless select, the "turbo button" is pressed20
			// Then it's 80%
		...
		if (gamepad.getRawButton(X_BUTTON)) {
			ballSpinner.set(locked ? 0.0 : -0.6);
		...
		if (latchB) {
			...
			latch.set(Math.abs(gamepad.getRawAxis(5)) > 0.1 ? gamepad.getRawAxis(5) * -1 : 0);
		}
