...
public class DriveControls {
	...
	private double SPEED_MULTIPLIER = 0.7;
	...
	private void arcadeDrive() {
		if (drivepad.getRawButton(9) || drivepad.getRawButton(10)) {
			SPEED_MULTIPLIER = .8;
		}
		...
		if (!linear && Math.abs(drivepad.getRawAxis(RIGHT_ANALOG)) <= 0.1) {
			if (drivepad.getRawButton(LEFT_BUMPER)) {
				leftFront.set(SPEED_MULTIPLIER * -1); 
				leftBack.set(0);
				rightFront.set(SPEED_MULTIPLIER * 1);
				rightBack.set(0);
	...
	public Double getRightBackV() {
		return rightBack.getOutputVoltage();
	}
	
	public Double getRightBackI() {
